...
SQL queries for digging into eBay-WooCommerce sync issues. All queries use
`{prefix}` — run `wp config get table_prefix` first.

## Find All Orders With eBay Metadata

```sql
SELECT p.ID, p.post_status, p.post_date,
       pm.meta_key, LEFT(pm.meta_value, 60) as meta_val
FROM {prefix}posts p
JOIN {prefix}postmeta pm ON p.ID = pm.post_id
WHERE p.post_type = 'shop_order'
  AND pm.meta_key LIKE '%ebay%'
ORDER BY p.post_date DESC
LIMIT 30;
```

## Find Orphaned Orders (eBay ID exists, bad WC status)
...
JOIN {prefix}postmeta pm ON p.ID = pm.post_id
WHERE p.post_type = 'shop_order'
  AND pm.meta_key LIKE '%ebay%order%id%'
  AND p.post_status IN ('wc-failed', 'wc-cancelled', 'wc-pending')
ORDER BY p.post_date DESC;
```

## Find Duplicate eBay Order IDs

```sql
SELECT pm.meta_value as ebay_order_id, COUNT(*) as order_count,
       GROUP_CONCAT(p.ID) as wc_order_ids
FROM {prefix}postmeta pm
JOIN {prefix}posts p ON pm.post_id = p.ID
WHERE pm.meta_key LIKE '%ebay%order%id%'
...
HAVING email IS NULL OR email = ''
   OR ship_name IS NULL OR ship_name = ''
ORDER BY p.post_date DESC
LIMIT 20;
```
...
Different plugins store eBay data under different meta keys. Find what's in use:
...
JOIN {prefix}posts p ON pm.post_id = p.ID
WHERE p.post_type = 'shop_order'
  AND (pm.meta_key LIKE '%ebay%' OR pm.meta_key LIKE '%ced%'
       OR pm.meta_key LIKE '%codisto%')
...
Orders that imported but have no line items are broken:
...
```sql
SELECT a.action_id, a.hook, a.status,
       a.scheduled_date_gmt, a.last_attempt_gmt
FROM {prefix}actionscheduler_actions a
WHERE a.hook LIKE '%ebay%'
...
# Get full order meta for a specific WC order
